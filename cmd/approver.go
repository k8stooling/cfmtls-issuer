...
func main() {
	...
	ApproverController := &controllers.ApproverController{}
	
	if err := ApproverController.SetupWithManager(mgr); err != nil {
	  setupLog.Error(err, "unable to set up controller")
		 os.Exit(1)
	} else {
		 setupLog.Info("ApproverController set up successfully")
	}
